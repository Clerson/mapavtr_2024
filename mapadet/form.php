    ...
    <div class="col-sm-6 mx-auto p-2">
      ...
      <form action='?p=<?=$p;?>&idmapa=<?=$idmapa;?>&acao=env' method='POST' class='row gx-2 gy-2 text-center'>
          ...
          <div class="input-group" id='odomentr'>
            <span class="input-group-text bg-info"><i class="fas fa-tachometer-alt"></i></span>
            <span class="input-group-text bg-info">Saída:</span>
            <input type="number" class="form-control shadow"  name="odomsaida" value="<?=$odomsaida;?>" placeholder="KM saída" required>
            <span class="input-group-text bg-info">Chegada:</span>
            <input type="number" class="form-control shadow"  name="odomentr" value="<?=$odomentr;?>" min="<?=$odomsaida;?>" placeholder="kM chegada" required>
          </div>

          <div class="input-group">
            <span class="input-group-text bg-info"><i class='far fa-hourglass'></i></span>
            <span class="input-group-text bg-info">Saída:</span>
            <input type="time" class="form-control shadow" name="horasaida"  value="<?=$horasaida;?>" required>
            <span class="input-group-text bg-info">Chegada:</span>
            <input type="time" id="horaentr" class="form-control shadow"  name="horaentr" value="<?=$horaentr;?>" required>
          ...
          <div class="col-sm form-floating" id="rai">
            <input type='int' class='form-control input-group-text shadow' name='num_rai' value='<?=$num_rai;?>'>
            <label for='num_rai'>Num RAI:</label>
          </div>

        <div class="col-sm form-floating"> 
          <select class="form-select shadow" name="status" id="status" onchange="mudaCorStatus(this.value)" required>
        ...
        <div class="form-floating"> 
          <textarea rows="5" cols="50" class="form-control shadow" name="obs" placeholder="obs"><?=$obs;?></textarea>
        ...
        <div class="form-floating">
          <?php if(!empty($_GET['iddetmp'])) { ;?>
            <a href="?p=<?=$p;?>&acao=ins&idmapa=<?=$idmapa;?>&vtrtipo=<?=$vtrtipo;?>&idpessoa=<?=$idpessoa;?>&odomsaida=<?=$odomentr;?>&vtrimg=<?=$vtrimg;?>&pessoaimg=<?=$pessoaimg;?>" class="btn btn-primary shadow"><i class="fas fa-plus-circle"></i> NOVA SAÍDA</a>
          <?php } ;?>
          
          <button type="submit" class="btn btn-success shadow"><i class="far fa-thumbs-up"></i> SALVAR</button>
        </div>
      </form>
 </div> 
